...
import type { Context, MatchSnapshotConfig } from 'jest-snapshot/build/types'
...
export { SnapshotState } from './State'
import * as utils from './utils'

export const EXTENSION = 'snap'
const DID_NOT_THROW = 'Received function did not throw' // same as toThrow
const NOT_SNAPSHOT_MATCHERS = `Snapshot matchers cannot be used with ${BOLD_WEIGHT('not')}`

const INDENTATION_REGEX = /^([^\S\n]*)\S/m

// Display name in report when matcher fails same as in snapshot file,
// but with optional hint argument in bold weight.
const printSnapshotName = (concatenatedBlockNames = '', hint = '', count: number): string => {
...
export const cleanup = () => {
  postSync('snapshot', { method: 'cleanup', filename: reporter.filename })
}

const toMatchSnapshot = function (
  this: Context,
  received: unknown,
  propertiesOrHint?: Record<string, any> | Config.Path,
  hint?: Config.Path,
): ExpectationResult {
  ...
  if (length === 2 && typeof propertiesOrHint === 'string') {
    ...
    if (typeof propertiesOrHint !== 'object' || propertiesOrHint === null) {
      ...
      let printedWithType = printWithType('Expected properties', propertiesOrHint, printExpected)
...
const toMatchInlineSnapshot = function (
  this: Context,
  received: unknown,
  propertiesOrSnapshot?: Record<string, any> | string,
...
const _toMatchSnapshot = (config: MatchSnapshotConfig) => {
  const { context, hint, inlineSnapshot, isInline, matcherName, properties } = config
  let { received } = config
  ...
  if (isNot) {
    throw new Error(matcherErrorMessage(matcherHintFromConfig(config, false), NOT_SNAPSHOT_MATCHERS))
  }
  ...
  if (typeof properties === 'object') {
    if (typeof received !== 'object' || received === null) {
      throw new Error(
        matcherErrorMessage(
          matcherHintFromConfig(config, false),
          `${RECEIVED_COLOR('received')} value must be an object when the matcher has ${EXPECTED_COLOR(
            'properties',
          )}`,
...
const _toThrowErrorMatchingSnapshot = (config: MatchSnapshotConfig, fromPromise?: boolean) => {
  ...
  if (error === undefined) {
    ...
    throw new Error(matcherHintFromConfig(config, false) + '\n\n' + DID_NOT_THROW)
  }
