...
export default function patchStackTrace (jasmineRequire: any) {
  jasmineRequire.StackTrace = () => StackTrace
}
...
class StackTrace {
  ...
  constructor (error: { stack: string; message: any }) {
...
function messagePrefixLength (message, stackLines) {
  ...
  for (i = 1; i < messageLines.length; i++) {
    if (messageLines[i] !== stackLines[i] && 'Error: ' + messageLines[i] !== stackLines[i]) {
      return 0
    }
  }
